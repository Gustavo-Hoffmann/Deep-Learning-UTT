...
from etapa01_setup import ExperimentConfig, build_default_config, create_output_dirs, device_supports_non_blocking, get_device, set_seed
from timing_utils import format_duration
...
class FoldTrainingResult:
    ...
    window_start_times: np.ndarray | None = None
...
def train_one_epoch(
    ...
    *,
    ...
    use_amp: mixed precision opcional (CUDA/ROCm). Cai para FP32 se falhar.
    """
    model.train()
    losses: list[float] = []
    nb = device_supports_non_blocking(device)
    amp_on = use_amp and device.type == "cuda" and amp_enabled is not False
    ...
    for batch in loader:
        ...
        if amp_on and grad_scaler is not None:
            try:
...
def create_grad_scaler(device: torch.device, use_amp: bool) -> tuple[torch.cuda.amp.GradScaler | None, bool]:
    """Cria GradScaler para AMP; retorna (scaler, amp_habilitado)."""
    if not use_amp or device.type != "cuda":
        return None, False
    try:
        return torch.cuda.amp.GradScaler(), True
    except Exception:
        return None, False
...
def train_single_fold(
    ...
    grad_scaler, amp_ok = create_grad_scaler(device, getattr(train_cfg, "use_amp", False))
    if getattr(train_cfg, "use_amp", False) and not amp_ok:
        print("  Aviso: --amp solicitado mas indisponível; usando FP32.")
    ...
    val_times: list[np.ndarray] = []
    for batch in loaders.val:
        val_times.append(batch["window_start_time"].detach().cpu().numpy())
    window_start_times = np.concatenate(val_times) if val_times else None
