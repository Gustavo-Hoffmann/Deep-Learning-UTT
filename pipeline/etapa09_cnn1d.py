...
class CNN1DRegressor(nn.Module):
    """
    CNN 1D simples: IMU (6 × T) -> amplitude escalar do Vicon.

    Entrada  : (batch, n_canais, tamanho_janela)
    Saída    : (batch,) — um valor por janela

    Pipeline interno
    ----------------
    Conv1d extrai padrões locais no tempo (picos, oscilações, transients)
    -> pooling comprime a dimensão temporal
    -> AdaptiveAvgPool resume toda a janela num vetor fixo
    -> Linear mapeia features -> amplitude predita
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Parameters
        ----------
        x : Tensor (batch, canais, tempo)

        Returns
        -------
        Tensor (batch,) — amplitude predita por janela
        """
