...
class TCNRegressor(nn.Module):
    """
    TCN: IMU (6 × T) -> amplitude escalar do Vicon.

    Entrada  : (batch, n_canais, tamanho_janela)
    Saída    : (batch,)

    Diferença em relação à CNN 1D (Etapa 9)
    ----------------------------------------
    - CNN 1D usa pooling → perde resolução temporal
    - TCN usa dilatação → expande campo receptivo mantendo resolução
    - Conexões residuais facilitam treino de pilhas profundas
    """
