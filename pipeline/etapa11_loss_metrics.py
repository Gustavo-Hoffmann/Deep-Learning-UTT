...
def compute_regression_metrics(
    ...
    """
    Calcula métricas sobre arrays 1D (mesma escala para true e pred).

    mae        : erro absoluto médio
    rmse       : penaliza erros grandes (como MSE, mas na mesma unidade do alvo)
    r2         : fração da variância explicada (1.0 = perfeito)
    bias       : média(y_pred - y_true) — superestima (+) ou subestima (-)
    mape_pct   : erro percentual absoluto médio (%)
    """
    yt = np.asarray(y_true, dtype=float).ravel()
    yp = np.asarray(y_pred, dtype=float).ravel()

    if len(yt) != len(yp):
        raise ValueError(f"Tamanhos diferentes: y_true={len(yt)}, y_pred={len(yp)}")
    if len(yt) == 0:
    ...
    mape = float(np.mean(np.abs(yp - yt) / (np.abs(yt) + epsilon)) * 100.0)
...
def metrics_to_original_scale(
    ...
    """
    Reverte predições/alvos para unidade original (ex.: cm).

    Use quando o vicon foi normalizado na Etapa 6.
    """
    yt = scaler_bundle.inverse_transform_target(y_true_scaled)
    yp = scaler_bundle.inverse_transform_target(y_pred_scaled)
    return yt, yp
