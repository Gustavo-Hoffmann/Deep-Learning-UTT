...
from etapa11_loss_metrics import TrainingConfig
...
def run_loso_on_dev(
    ...
    per_subject: list[SubjectFoldMetrics] = []
    global_best_mae = global_best_val_mae_cm
    pruned_config = False
    ...
    for i, val_sid in enumerate(val_ids, start=1):
        ...
        result = train_single_fold(
            ...
            global_best_val_mae=global_best_mae,
        )

        if result.history.stop_reason == "pruning":
            pruned_config = True
            print(f"  → configuração interrompida por pruning no fold {val_sid}\n")
            break

        if global_best_mae is None:
            global_best_mae = result.best_val_metrics.mae
        else:
            global_best_mae = min(global_best_mae, result.best_val_metrics.mae)
        ...
        print(
            f"  → MAE={sub_metrics.mae:.4f} | RMSE={sub_metrics.rmse:.4f} | "
...
def _save_oof_predictions(
    ...
    """Salva predições out-of-fold em cm para calibração e Bland-Altman LOSO."""
    from analysis_utils import collect_oof_predictions_cm

    rows: list[dict] = []

    # Preferir arrays em memória; senão reconstruir a partir dos checkpoints (retomada).
    for result in fold_results:
        ...
        from etapa11_loss_metrics import metrics_to_original_scale
        ...
        for i in range(len(yt_cm)):
            rows.append(
                {
                    "subject_id": result.val_subject_id,
                    "y_true_cm": float(yt_cm[i]),
                    "y_pred_cm": float(yp_cm[i]),
                    "error_cm": float(yp_cm[i] - yt_cm[i]),
                }
            )
