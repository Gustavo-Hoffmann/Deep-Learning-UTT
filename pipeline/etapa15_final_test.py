...
from etapa12_train_fold import build_model_from_config, train_one_epoch
...
def collect_test_predictions(
    ...
    """Coleta predições janela a janela com subject_id e escala original."""
    model.eval()
    rows: list[dict] = []
    offset = 0
    ...
    for batch in loader:
        ...
        yhat_np = y_hat.detach().cpu().numpy().ravel()
        n = len(y_np)
            ...
            rows.append(
                {
                    ...
                    "window_start_idx": int(test_batch.window_start_idx[idx]),
                    "y_true_scaled": float(y_np[i]),
                    "y_pred_scaled": float(yhat_np[i]),
                    "y_true_cm": float(y_cm[i]),
                    "y_pred_cm": float(yhat_cm[i]),
                    "error_cm": float(yhat_cm[i] - y_cm[i]),
                }
            )
        offset += n
...
def run_stage15_final_test(
    ...
    best_hparams = load_best_hyperparameters(paths["configs"]) if use_best_hparams else None
    if best_hparams:
    ...
    test_loader = create_dataloader(test_ds, batch_size=train_cfg.batch_size, shuffle=False, num_workers=train_cfg.num_workers)
    ...
    train_losses = train_final_model(
        model, train_loader, criterion, optimizer, device, n_epochs=n_epochs, verbose=verbose
    )
    ...
    pred_df = collect_test_predictions(model, test_batch, test_loader, device, scaler)
    ...
    eval_paths = save_window_and_subject_metrics(
        pred_df, paths["metrics"], paths["plots"], prefix="test"
    )
