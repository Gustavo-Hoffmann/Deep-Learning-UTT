#!/usr/bin/env python3
"""Gera documentação Markdown automática do pipeline."""
...
def generate_documentation(
    ...
    *,
    ...
    c = context
    md = f"""# Documentação — Pipeline UTT Deep Learning Residual

Gerado automaticamente em `{out_path.name}`.
...
- **{c.get('n_subjects', '?')}** sujeitos no total
- Treino: {c.get('train_subjects', [])}
- Validação: {c.get('val_subjects', [])}
- Seed: {c.get('seed', 42)}
...
Coeficientes: a = {c.get('baseline_slope', '?'):.6f}, b = {c.get('baseline_intercept', '?'):.6f}
...
- Parâmetros: {c.get('n_params', '?')}

## 11. Janelamento temporal

- `window_size = {c.get('window_size', 512)}`
- `stride = {c.get('stride', 128)}`
...
Device usado nesta execução: **{c.get('device', '?')}** ({c.get('device_desc', '')})
...
| Etapa | Tempo |
|-------|-------|
| Leitura dos dados | {c.get('data_load_s', 0):.1f} s |
| Treinamento | {c.get('train_s', 0):.1f} s |
| Avaliação/inferência | {c.get('eval_s', 0):.1f} s |
| Script total | {c.get('total_s', 0):.1f} s |
| Melhor época | {c.get('best_epoch', '?')} |
| Melhor val_loss | {c.get('best_val_loss', '?')} |
| Melhor val_RMSE | {c.get('best_val_rmse', '?')} cm |

### Resultados desta execução

| Método | RMSE médio (cm) |
|--------|-----------------|
| Smart raw | {c.get('rmse_raw', float('nan')):.4f} |
| Linear | {c.get('rmse_linear', float('nan')):.4f} |
| DL residual | {c.get('rmse_dl', float('nan')):.4f} |
| Melhora DL vs linear | {c.get('improvement_pct', float('nan')):.2f}% |
"""
